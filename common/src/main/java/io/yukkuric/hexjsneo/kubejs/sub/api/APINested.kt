...
class APINested(f: ContextFactory, private val src: ClassWalkResult) : HexJSPluginObject(f) {
    ...
    override fun getCustom(cx: Context, name: String, start: Scriptable): Any? {
        ...
        src.subPackages[name]?.let { p -> return cacheSub.computeIfAbsent(name) { APINested(factory, p) } }
        return NOT_FOUND
    }
